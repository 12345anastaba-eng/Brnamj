...
# OCR fallback for scanned PDFs or PDFs using a custom/embedded font
# whose internal glyph codes don't map to real Unicode characters
# (a very common issue with Arabic PDFs, producing text like "/C142").
try:
...
def is_text_garbled(text: str) -> bool:
    """
    Heuristic check for PDFs whose embedded font has no proper
    Unicode mapping. In that case pypdf leaks raw glyph codes like
    '/C142' instead of real Arabic letters, and the ratio of actual
    Arabic characters to total characters is very low.
    """
    ...
    arabic_ratio = arabic_chars / len(stripped)
    glyph_leaks = len(re.findall(r"/C\d+", text))
    return arabic_ratio < 0.3 or glyph_leaks > 5


def extract_text_from_pdf_via_ocr(file_bytes: bytes) -> str:
    """Render each PDF page as an image and run Arabic OCR on it."""
    if convert_from_bytes is None or pytesseract is None:
        st.error(
            "OCR fallback needs 'pdf2image' + 'pytesseract' in requirements.txt, "
            "and 'poppler-utils' + 'tesseract-ocr' + 'tesseract-ocr-ara' in packages.txt."
        )
        return ""
    try:
        images = convert_from_bytes(file_bytes, dpi=300)
    except Exception as e:  # noqa: BLE001
        st.error(f"Could not render PDF pages for OCR: {e}")
        return ""

    pages_text = []
    for image in images:
        try:
            pages_text.append(pytesseract.image_to_string(image, lang="ara"))
        except Exception as e:  # noqa: BLE001
            st.error(f"OCR failed on a page: {e}")
    return "\n".join(pages_text)
...
def extract_text_from_pdf(uploaded_file) -> str:
    ...
    if is_text_garbled(text):
        st.warning(
            "تعذّرت القراءة المباشرة لنص الملف (يبدو أن الملف يستخدم خطًا مخصصًا). "
            "جارٍ استخدام التعرف الضوئي على الحروف (OCR)، قد يستغرق ذلك بضع ثوانٍ..."
        )
        ocr_text = extract_text_from_pdf_via_ocr(file_bytes)
        if ocr_text.strip():
            return ocr_text
        st.error(
            "تعذّر استخراج نص واضح من هذا الملف حتى باستخدام OCR. "
            "جرّب رفعه كملف .txt أو .docx بدلاً من PDF إن أمكن."
        )
        return ""
